import { parsePNR } from './src/lib/parser';
        ...
        if (result) {
            ...
            if (result.quotations && result.quotations.length > 0) {
                result.quotations.forEach((quote, idx) => {
                    ...
                    if (quote.fares && quote.fares.length > 0) {
                        quote.fares.forEach(fare => {
                            console.log(`     * ${fare.category}/${fare.paxType || 'ADT'}: Tarifa ${fare.tarifa} + Taxas ${fare.taxas}`);
                        });
                    }
                    console.log(`   - Trechos: ${quote.trechos?.length || 0}`);
                    quote.trechos?.forEach((trecho, i) => {
                        console.log(`     ${i + 1}. ${trecho}`);
                    });
                    console.log(`   - Segments processados: ${quote.segments?.length || 0}`);
                    console.log(`   - Payment Terms: ${quote.paymentTerms || 'N/A'}`);
                    console.log(`   - Baggage: ${quote.baggage || 'N/A'}`);
                    console.log(`   - Notes: ${quote.notes || 'N/A'}`);
                    console.log(`   - Parcelas: ${quote.numParcelas || 1}`);
