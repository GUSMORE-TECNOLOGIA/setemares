...
// Dados completos de aeroportos do mundo (1000+ aeroportos)
const worldAirports = [
...
async function importAirports() {
  console.log('Importando aeroportos do mundo...');
  
  const airportsData = worldAirports.map(airport => ({
    iata3: airport.iata,
    icao4: airport.icao,
    name: airport.name,
...
async function importCities() {
  console.log('Importando cidades do mundo...');
...
async function main() {
  try {
    console.log('🚀 Iniciando importação MEGA de dados...\n');
    
    await importCities();
    await importAirports();
    
    console.log('\n🎉 Importação MEGA concluída com sucesso!');
    console.log(`📊 Total: ${worldAirports.length} aeroportos e cidades do mundo!`);
  } catch (error) {
